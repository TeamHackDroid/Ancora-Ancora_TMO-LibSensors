...
#define BMA_MAGCALIB_SHAPE_NUM              (2)
...
struct bma_mag_data {
    struct bma_vector xyz; /* without offset, filtered */
    struct bma_vector raw; /* with offset, not filtered */
    struct bma_vector xy1y2;
    int16_t temperature;
};

struct bma_mag_offset {
    int8_t hard_offset[3];
    struct bma_vector calib_offset;
};
struct bma_mag_status {
    struct bma_mag_offset offset;
    int accuracy;
};
struct bma_offset {
    struct bma_mag_status mag[BMA_MAGCALIB_SHAPE_NUM];
};

struct bma_mag_driver_callback {
    int (*lock)(void);
    int (*unlock)(void);
    int (*i2c_open)(void);
    int (*i2c_close)(void);
...
struct bma_mag_driver {
    int (*init)(void);
    int (*term)(void);
    int (*get_delay)(void);
    int (*set_delay)(int msec);
    int (*get_offset)(struct bma_mag_offset *offset);
    int (*set_offset)(struct bma_mag_offset *offset);
    int (*get_enable)(void);
    int (*set_enable)(int enable);
    int (*get_filter)(struct bma_mag_filter *filter);
    int (*set_filter)(struct bma_mag_filter *filter);
    ...
    int (*measure)(struct bma_mag_data *data, int *time_delay_ms);
    struct bma_mag_driver_callback callback;
};

struct bma_mag_calibration_result {
    int32_t spread;
    int32_t variation;
    int32_t radius;
    int8_t axis;
...
struct bma_mag_calibration_threshold {
    int32_t spread;
    int32_t variation[3];
};

struct bma_mag_calibration_callback {
    int (*lock)(void);
    int (*unlock)(void);
};

struct bma_mag_calibration {
    int (*init)(void);
    int (*term)(void);
    int (*update)(struct bma_vector *mag,
            struct bma_mag_calibration_result *result);
    ...
    int (*get_threshold)(struct bma_mag_calibration_threshold *threshold);
    int (*set_threshold)(struct bma_mag_calibration_threshold *threshold);
    struct bma_mag_calibration_callback callback;
};
...
struct bma_utility {
    int (*get_rotation_matrix)(struct bma_vector *acc, struct bma_vector *mag,
            struct bma_matrix *matrix);
    int (*get_euler)(struct bma_matrix *matrix, struct bma_vector *euler);
};
...
int bma_acc_driver_init(struct bma_acc_driver *f);
int bma_acc_calibration_init(struct bma_acc_calibration *f);
int bma_utility_init(struct bma_utility *f);
